...
class Program():
    ...
    def kolesa(self, power_factor):
        ...
        if (bool(gamepad.isPadPressed(1))):
            ...
            if (self.x != 0 and self.y == 0):
                # лево/право
                brick.motor("M2").setPower(self.x * power_factor)
                brick.motor("M1").setPower(-self.x * power_factor)
            elif (self.y != 0):
                # врепёд/назад
                brick.motor("M1").setPower(self.y * power_factor)
                brick.motor("M2").setPower(self.y * power_factor)
    ...
    def execMain(self):
        ...
        while True:
            ...
            if gamepad.buttonWasPressed(2):
                power_factor = 0.6
            elif gamepad.buttonWasPressed(3):
                power_factor = 1.0            

            # отключаем сервопривод, отвечающий за зажатие
            if (gamepad.buttonWasPressed(3)):
              brick.motor("S2").power0ff()
